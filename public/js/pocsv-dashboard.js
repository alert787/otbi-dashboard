const POCSVDashboard = {
    ...
    updateCharts(chartsData) {
        ...
        try {
            const byStatus = chartsData.by_status || {};
            if (Object.keys(byStatus).length > 0) {
                this.renderChart('chart-status', 'donut', Object.keys(byStatus), Object.values(byStatus), null);
            }

            let byJenis = chartsData.by_jenis_po || {};
            if (Object.keys(byJenis).length === 0) {
                byJenis = chartsData.by_jenis_pengadaan || {};
            }
            if (Object.keys(byJenis).length > 0) {
                this.renderChart('chart-jenis', 'donut', Object.keys(byJenis), Object.values(byJenis), null);
            }
            ...
            const byBU = chartsData.by_bu || {};
            const sortedBU = Object.entries(byBU).sort(([, a], [, b]) => b - a).slice(0, 10);
            if (sortedBU.length > 0) {
                this.renderChart('chart-bu', 'bar', sortedBU.map(([label]) => label), sortedBU.map(([, value]) => Math.round(value)), (v) => this.fmtCurrency(v));
            }
            ...
            buttons: [
                {
                    ...
                    title: 'Laporan PO (CSV)',
                    filename: () => `laporan_po_csv_${document.getElementById('filterTahun').value || 'all'}`
                },
                {
                    extend: 'csvHtml5',
                    text: 'Export CSV',
    ...
    async loadData() {
        ...
        try {
            const res = await axios.get(this.BASE_URL + 'pocsv/data', { params: { tahun } });
            const d = res.data;
